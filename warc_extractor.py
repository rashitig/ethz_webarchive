...
def warc_records(search_string, path):
    """Iterates over warc records in path."""
    for filename in os.listdir(path):
        if re.search(search_string, filename) and ".warc" in filename:
...
def parse(args):
    ...
    for record, filename in warc_records(args.string, args.path):
        try:
            ...
            if args.dump == "content":
                ...
                path = args.output_path + filename +"_"+ host + path
